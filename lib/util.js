...
function checkPhantomjsVersion(phantomPath) {
  ...
  return kew.nfcall(cp.execFile, phantomPath, ['--version']).then(function (stdout) {
    ...
    if (helper.platform && helper.platform.indexOf('freebsd') > -1 ||
        (process.platform && process.platform.indexOf('freebsd') > -1)) {
      console.log('Ravn hack: ignore version validation check if platform is freebsd. Found installed version ' + version +
          ' and module is looking for ' + helper.version);
      return true;
    }
